        ...
        <div class="container-fluid px-0" style="padding-top:6px;">
            <style>
                ...
                @php
                    ...
                    $headWarehouse = null;
                    if ($headOfficeId) {
                        try { $headWarehouse = \App\Models\Warehouse::find($headOfficeId); } catch (\Exception $e) { $headWarehouse = null; }
                    }
                    if (!$headWarehouse) { $headWarehouse = $invoice->warehouse; }

                    // prepare logo url early so we can place logo at the top area
                    // Prefer head office logo; fallback to invoice location
                    $candidateLogoTop = null;
                    if (!empty($headWarehouse)) {
                        $candidateLogoTop = $headWarehouse->image ?? $headWarehouse->logo ?? $headWarehouse->logo_url ?? $headWarehouse->image_url ?? null;
                    }
                ...
                <!-- Header: split into two halves: left=head office, center divider, right=selected location -->
                <div style="width:100%; display:flex; gap:12px; align-items:flex-start; border:1px solid #000; padding:8px; box-sizing:border-box;">
                    <div style="flex:1; text-align:left;">
                        {{-- Head Office Details (left) --}}
                        @php
                            $headName = $headWarehouse->company_name ?? $headWarehouse->name ?? config('app.name');
                        @endphp
                        <div style="font-weight:700; font-size:18px;">{{ $headName }}</div>
                        @if(!empty($headWarehouse->sub_heading))
                            <div style="font-size:13px; margin-top:4px;">{{ $headWarehouse->sub_heading }}</div>
                        @endif
                        <div style="margin-top:6px; font-size:12px;">{{ $headWarehouse->address ?? '' }}</div>
                        <div style="margin-top:4px; font-size:12px;">Mobile: {{ $headWarehouse->mobile ?? '' }} &nbsp; Email: {{ $headWarehouse->email ?? '' }}</div>
                        <div style="margin-top:4px; font-size:12px;">GSTIN / UIN: {{ $headWarehouse->gstn_uin ?? $headWarehouse->gst_no ?? '' }}</div>
                    </div>

                    <!-- centered vertical divider -->
                    <div style="width:0; border-left:1px solid #000; align-self:stretch;"></div>

                    <div style="flex:1; text-align:left;"> <!-- right column left-aligned per request -->
                        {{-- Selected Location Details (right) --}}
                        @php
                            $loc = $invoice->warehouse ?? null;
                            $locName = $loc->company_name ?? $loc->name ?? 'Location';
                        @endphp
                        <div style="font-weight:700; font-size:16px;">{{ $locName }}</div>
                        @if(!empty($loc->sub_heading))
                            <div style="font-size:12px; margin-top:4px;">{{ $loc->sub_heading }}</div>
                        @endif
                        <div style="margin-top:6px; font-size:12px;">{{ $loc->address ?? '' }}</div>
                        <div style="margin-top:4px; font-size:12px;">Mobile: {{ $loc->mobile ?? '' }} &nbsp; Email: {{ $loc->email ?? '' }}</div>
                        <div style="margin-top:4px; font-size:12px;">GSTIN / UIN: {{ $loc->gstn_uin ?? $loc->gst_no ?? '' }}</div>
                    </div>
                </div>
                    ...
                    <table style="width:100%; border-collapse:collapse; font-size:13px;">
                        ...
                        <tbody>
                            ...
                            @foreach($invoice->items as $item)
                                    ...
                                    $total = $item->total ?? ($gstInclusiveUnit * $qty);
                        ...
                        <table style="width:100%; border-collapse:collapse; font-size:13px;">
                            <tbody>
                                @php
                                    ...
                                    // Net (base) amount should be the taxable sum. Total Invoice Value = Net + Tax.
                                    $displayGrandTotal = $grandTaxable; // base taxable amount
                                    $displayInvoiceValue = $displayGrandTotal + $displayTotalTax; // net + tax
                                    // Round Net Payable to nearest rupee (integer) and show round-off as the difference.
                                    $displayNetPayable = round($displayInvoiceValue); // integer rupees
                                    $displayRoundOff = $displayNetPayable - $displayInvoiceValue;
                                @endphp
                                <tr>
                ...
                <div style="display:flex; gap:12px; margin-top:12px;">
                    <div style="flex:1.4; border:1px solid #000; padding:8px;">
                    ...
                    <div style="flex:0 0 460px; border:1px solid #000; padding:8px;">
            ...
            <script>
                (function(){
                    ...
                    function doPrintThreeCopies() {
                        ...
                        var companyHeader = invoiceContainer.querySelector('div[style*="display:flex; gap:12px; align-items:flex-start; border:1px solid #000;"]');
