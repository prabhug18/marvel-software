...
<tr>
    ...
    <td class="action-buttons"> 
        ...
        @if(auth()->user() && auth()->user()->hasRole('Admin'))
            <form id="delete-form-{{ $productVal->id }}" method="POST" action="{{ route('products.destroy', $productVal->id) }}" class="d-inline">
                @csrf
                @method('DELETE')
                ...
                @else
                    <button type="button" class="btn btn-sm btn-outline-danger btn-delete-product" data-id="{{ $productVal->id }}"><i class="fas fa-trash-alt"></i></button>
                @endif
            </form>
        @endif
    </td>
